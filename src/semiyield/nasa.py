...
from semiyield.visuals import write_degradation_chart
...
def convert_nasa_matlab_archive(
    ...
    *,
    ...
    with zipfile.ZipFile(source) as zipped, tempfile.TemporaryDirectory(
        prefix="semiyield-nasa-mat-"
    ) as temporary:
    ...
    *,
    ...
    for device_id, device_frame in working.groupby(mapping.device, sort=True):
        ...
        if "run_id" in device_frame:
            ...
            baseline_values = device_frame.loc[
                device_frame["run_id"].eq(first_run), "rds_on_ohm"
            ]
        else:
...
def derive_lifetime_table(
    ...
    *,
    ...
    for device, group in features.sort_values("time_end_s").groupby("device_id", sort=True):
        ...
        persistent = above.rolling(persistence_windows, min_periods=persistence_windows).sum().eq(
            persistence_windows
        )
        reached_positions = np.flatnonzero(persistent.to_numpy())
        event = bool(len(reached_positions))
        endpoint_position = (
            max(0, int(reached_positions[0]) - persistence_windows + 1)
            if event
            else len(group) - 1
        )
