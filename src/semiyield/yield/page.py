...
from .metrics import threshold_table
...
def render(zh):
    ...
    try:
        ...
        st.error(
            f"请先运行 `semiyield yield download`: {exc}"
            if zh
            else f"Run `semiyield yield download` first: {exc}"
        )
        st.stop()
    if page == "quality":
        ...
        if page == "benchmark":
            ...
            if report_path.exists():
                ...
                st.info(
                    "运行 `semiyield yield benchmark --profile quick` 生成模型对比。"
                    if zh
                    else "Run `semiyield yield benchmark --profile quick` for a fair comparison."
                )
        elif page == "threshold":
